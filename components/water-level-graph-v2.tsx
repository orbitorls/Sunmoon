...
import { TideData } from "@/lib/tide-service";
...
export const WaterLevelGraphV2: React.FC<WaterLevelGraphProps> = ({ tideData, location }) => {
    ...
    return (
      <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-lg dark:border-slate-700 dark:bg-slate-950">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          เทียบอ้างอิง: <span className={cn("font-medium", diff > 0 ? "text-orange-500" : "text-emerald-500")}>
  ...
  return (
    <Card className="w-full overflow-hidden border-0 bg-white/90 shadow-lg ring-1 ring-slate-900/5 dark:bg-slate-950/80">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid min-w-[220px] grid-cols-2 gap-3">
              <div className="rounded-xl border border-white/20 bg-white/10 px-3 py-2 backdrop-blur-sm">
                ...
                <div className="text-base font-bold">{tideData.highTideTime || "--:--"}</div>
              </div>
              <div className="rounded-xl border border-white/20 bg-white/10 px-3 py-2 backdrop-blur-sm">
                <div className="text-xs text-white/70">น้ำลงต่ำ</div>
                <div className="text-base font-bold">{tideData.lowTideTime || "--:--"}</div>
              </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {tideData.graphData.map((data, index) => {
                      ...
                      return (
                        <tr key={index} className={cn("transition-colors", isCurrent ? "bg-blue-50/60 dark:bg-blue-900/20" : "bg-white dark:bg-slate-950")}>
                          ...
                          <td className={cn("px-4 py-2.5 text-center font-mono", diff > 0 ? "text-orange-500" : "text-emerald-500")}>
                            {diff > 0 ? "+" : ""}{diff.toFixed(2)}
                          </td>
                          <td className="px-4 py-2.5 text-center">
                            <span className={cn("inline-block h-1.5 w-1.5 rounded-full mr-2", data.prediction ? "bg-blue-400" : "bg-emerald-500")} />
...
export default WaterLevelGraphV2;
