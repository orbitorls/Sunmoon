...
interface TideStatusHeroProps {
    ...
    /** Whether the level has been converted to mean sea level. `false` for
     *  every station today, so the label must not claim MSL unconditionally. */
    isDatumConvertedToMsl?: boolean;
    className?: string;
...
function ReportTideButton({
    ...
    const submit = async () => {
        ...
        try {
            ...
            setOpen(false);
            setLevelMeters("");
            toast.success("ขอบคุณสำหรับรายงาน! ข้อมูลจะถูกนำไปตรวจสอบก่อนนำไปใช้ปรับโมเดล");
        } catch (error) {
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent className="w-80 text-slate-900" align="end">
...
export function TideStatusHero({ status, currentLevel, nextEvent, className, isDatumConvertedToMsl, stationId, location, locationName }: TideStatusHeroProps) {
    ...
    const containerClass = cn(
        "relative overflow-hidden rounded-3xl p-4 shadow-2xl transition-all duration-700 motion-reduce:transition-none sm:p-6 border-4",
        ...
        <Card className={cn("border-none", containerClass)}>
            ...
            <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                ...
                <StatCard
                    ...
                    label={<><Droplets className="w-3 h-3 sm:w-4 sm:h-4" />ระดับน้ำปัจจุบัน{isDatumConvertedToMsl ? " (MSL)" : " (ระดับสถานี)"}</>}
                    labelAfter
                >
                    {locationName && (
                        <span className="mb-1 inline-flex max-w-[16rem] items-center gap-1.5 self-end rounded-full border border-white/10 bg-black/20 px-3 py-1 text-xs font-bold text-white backdrop-blur-md">
                            <MapPin className="h-3.5 w-3.5 shrink-0" />
                            <span className="truncate">{locationName}</span>
                        </span>
                    )}
                    <div className="flex items-baseline gap-2">
                        {/* Shown as computed. `adjustHeightForDatum` would apply a
                            global CD offset, but `lib/domain/datum.ts` records no
                            per-station MSL height, so the shifted number would be
                            wrong by a different amount at every station. */}
                        <span className="text-5xl sm:text-6xl md:text-7xl font-black tracking-tighter tabular-nums drop-shadow-2xl">
