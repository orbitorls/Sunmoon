 ...
 *
 * Split out of lib/disaster-analysis.ts (god module): risk classification and
 * the analyzeDisasterRisk entry point.
 */

import type { TideData, WeatherData, TideEvent } from './types';
import {
  THRESHOLDS,
  type AdvanceWarning,
...
export function analyzeDisasterRisk(
  ...
  // สร้างประโยคสรุปเฉพาะเจาะจง (เวลา + ระดับน้ำ + ลม)
  const headline = formatDisasterHeadline(
    { riskLevel, floodPrediction, advanceWarnings, riskTimeline, factors },
