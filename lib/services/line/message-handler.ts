...
export async function handleLineMessage(event: LineEvent): Promise<void> {
  try {
    ...
    try {
      await sendLineMessage(event.replyToken, [buildErrorMessage()])
    } catch (sendError) {
...
async function handleTextMessage(event: LineEvent, userId: string | null): Promise<void> {
  ...
  if (!location) {
    await sendLineMessage(event.replyToken, [buildUnsupportedLocationMessage()])
    return
  }
 ...
 */
export function parseLocationFromText(text: string): LocationData | null {
