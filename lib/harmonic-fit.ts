...
import { computeConstituentBasis } from './harmonic-tide-core'
 ...
 */
export function fitConstituents(samples: FitSample[], names: string[], longitude: number): FitResult {
  ...
  const constituents = names
    ...
    .filter((c) => c.amplitude >= MIN_AMPLITUDE_METERS)
