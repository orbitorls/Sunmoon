...
// through, so gating here protects all of them (tide-service.ts, the
...
// lib/tide-service.ts: a station constant without real-fit provenance is no
...
export function getStationHarmonicPrediction(
  ...
  const extremes = findHighLowTides(shiftedStart, shiftedEnd, constituents, 10, nearest.station.lon, epoch);
